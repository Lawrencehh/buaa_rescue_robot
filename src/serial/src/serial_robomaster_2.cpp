...
// 类定义
// 创建一个继承自rclcpp::Node的serial_robomaster_2类，并在构造函数中进行初始化。
class serial_robomaster_2 : public rclcpp::Node
{
public:
    serial_robomaster_2() : Node("serial_robomaster_2")
    {
        // 尝试初始化串口，如果出错，记录错误信息。
        try {
            serial_port_ = std::make_shared<asio::serial_port>(io_, "/dev/ttyRobomaster2");  // 这里的路径需要根据您的设备进行更改
...
private:    // 私有成员函数和变量
    ...
    int16_t gripper_gm6020_position;
    int16_t gripper_c610_position;
    int16_t gripper_sts3032_position; 
};
...
int main(int argc, char **argv) // 主函数
{
    ...
    auto serial_robomaster_2_node = std::make_shared<serial_robomaster_2>();   // 创建serial_robomaster_2对象

    // 创建一个新线程来运行ASIO的io_service
    std::thread asio_thread([&]() {
        serial_robomaster_2_node->get_io_service().run();
    });

    rclcpp::spin(serial_robomaster_2_node);  // 开始ROS的事件循环
